...
def get_filename():
    ...
    return f"ultra_sonic_sensor/Improved_usability/data_collection_v3.1/data_3.1/US_data_{filename}.csv"
...
def generate_sequence_filename(metadata):
    ...
    return f"ultra_sonic_sensor/Improved_usability/data_collection_v3.1/data_3.1/test_seq_ard{metadata['Arduino ID']}_sensor{metadata['Sensor ID']}_{current_date}.csv"
...
try:
    while True:
        user_input = input("Enter 'R' to record, 'M' to set delay in milliseconds, 'U' to set delay in microseconds, 'S' to run sequence, or 'q' to quit: ")
        if user_input.lower() == 'q':
            break
        elif user_input in ['R', 'M', 'U', 'S']:
